...
- **AI向けundo予算**: 非人間入口では、ボス区間ごとにundo回数を制限する(既定2回)。残数は観測と`undo`選択肢に表示される
...
| ツール | 役割 |
|---|---|
| `lok_new_run` | ラン開始(seed/ship/asc/contracts/undoLimit) |
| `lok_state` | 観測+合法手リスト |
...
```bash
node agent/cli.mjs new --seed 42 --ship vagrants [--contracts heavy,minefield] [--asc 1] [--undo-limit 2]
...
AI向け入口では`undo`はボス区間ごとに既定2回まで。`--undo-limit 0`で禁止、`--undo-limit unlimited`で旧来どおり無制限にできる。MCPでは`lok_new_run.undoLimit`で指定する。
