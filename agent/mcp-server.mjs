...
server.tool(
  ...
  "合法手リストのidを実行する(複数可=順に適用)。強制フェイズ(合法手1つ)は自動進行。sayで判断への一言実況を残せる(記事素材になる)",
  {
    ...
    wow: z.boolean().optional().describe("『今この瞬間が最高だった』マーク — 航海記録(chronicle)に★で刻まれる"),
  },
  async ({ ids, say, wow }) => {
    if (!S) return { content: [{ type: "text", text: "ランが無い — lok_new_run から始める" }] };
...
server.tool("lok_log", "ランの記録(シード・全選択ID・スコア・船長診断)を取得 — 記事/共有用", {}, async () => {
  ...
  const data = {
    ...
    chronicle: LK.voyageChronicle(S, LOG.wows || []),
    timeline,
...
server.tool(
  "lok_memory",
  "ラン終了後、記憶を2チャンネルで取得する。A=keepsake(想い出: 何をどう感じたか。あなた自身の記憶に保存・蒸留せよ) / B=carryover(引き継ぎ: 永続化して次回 lok_new_run の memory に蓄積して渡せば、次のランの冒頭で『記憶』として想起される)。noteにこのランの心残り/抱負を書くと次回の自分への伝言になる。",
  { note: z.string().optional().describe("このランの心残り/抱負(次のランの冒頭で自分に想起される自由文)") },
  async ({ note }) => {
    if (!S) return { content: [{ type: "text", text: "ランが無い" }] };
    const mem = LK.voyageMemory(S, LOG.says || [], LOG.wows || []);
    const out = {
      keepsake: { ...mem, prose: LK.voyageMemoryProse(mem) },
      carryover: LK.carryoverRecord(S, note || null),
    };
