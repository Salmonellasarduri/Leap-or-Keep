...
import { z } from "zod";
import { newGame, legalChoices, applyChoice, observe, LK } from "./protocol.mjs";

let S = null;
let LOG = { opts: null, ids: [] };
...
server.tool(
  "lok_choose",
  "合法手リストのidをひとつ実行する(例: pair:u0:top:u3 / act:0:ship:cell:2,1:right / commit / keep)",
  { id: z.string().describe("CHOICESに列挙されたid") },
  async ({ id }) => {
    if (!S) return { content: [{ type: "text", text: "ランが無い — lok_new_run から始める" }] };
    const legal = legalChoices(S);
    if (!legal.some(c => c.id === id) && !/^(leap:|loadout:)/.test(id))
      return { content: [{ type: "text", text: `✖ "${id}" は合法手にない。現在のCHOICES:\n` + legal.map(c => "  " + c.id).join("\n") }] };
    const r = applyChoice(S, id);
    if (!r.ok) return { content: [{ type: "text", text: `✖ 失敗: ${r.msg || "?"}` }] };
    LOG.ids.push(id);
    return { content: [{ type: "text", text: view(`✔ ${id}`) }] };
  }
);
...
server.tool("lok_log", "ランの記録(シード・全選択ID・スコア・船長診断)を取得 — 記事/共有用", {}, async () => {
  ...
  const data = {
    opts: LOG.opts, choices: LOG.ids.length, ids: LOG.ids,
