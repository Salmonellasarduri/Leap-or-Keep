...
### A. 視覚 / 部屋(最優先の続き)
- **A1 🔶 部屋Sol磨き 7.7→8.5** — Sol-driveワークフロー確立済み(§Sol協業レシピ②③)。残: 窓の遮蔽解消(キャニスター/HUDと重なる)/暖色プールの強度/タグリーダーのヒーロー化/黒潰れ残り。**場所**: §次タスク①(数値処方全文)。**完了**: ゲーム内 `?room=1` スクショで7.8+(Cyclesレンダの数字は信じない=L-040/L-044)。
- **A2 ⏳ 部屋を既定背景へ昇格** — A1が7.8+になったら。**場所**: holo.js bgShow既定をroomへ+META.room3dトグル(タイトル)+i18n追補。**完了**: 既定で部屋、2D画はフォールバック維持。
...
- **⚠ pre0 未着手(監査が全観点で検出したブロッカー)**: 集中型封印×1.5/×2.0と安全帰還+20%は **`cargoPayoutValue`(プレビュー/スコア)にしか無く、実精算(bonusValue×mult + 遺物ごとsellRelic value×mult)は1円も払っていない** = 既存バグ(プレイヤーに約束した額>実支給額)。憲章はこれらを本物の勝敗評価部品として設計しているので **案A=精算で本当に払う** が筋(Sol v2採用)。ただし収入~1.5倍の買い=balance再ベースライン必須+**オーナー判断が望ましい**。B1/B3はSol v2でpre0非依存に再設計済みなので先行可能だった。
- **B2 ⏳ オランダ式市場(①-3)** — Sol v2確定: 固定でなく**ランごとローテ**(一期一会)+圧縮値下げ(B/B-3/B-5床)+CB₢2累積+「買えたのに見送った(精算時credits≥価格)」時だけmisses++。⚠ META永続(marketMisses/marketSeen/marketFeatured)が要る。
- **B3 ⏳ 報酬の持参金+残念賞(③-3)** — Sol v2確定(3択化しない): 単一遺物に第3行動「見送り」を追加=**skip→dockRebate**(次のドック整備を値引く貯金プレイ、skipを死札にしない)。持参金は**展開時のみ**・条件遺物は₢0(situational性を守る)・平坦₢1は素の3遺物だけ。最大発行₢3/ラン。
...
- **GLB部屋**: `?room=1` の実験フラグで実動。**Sol磨きR3-R5実施済み(2026-07-14): 6.8→7.2→7.5→7.7**。既定背景は2D画(8.5)のまま — 昇格ゲート7.8にあと0.1。**7.8→8.5超えが残タスク**
