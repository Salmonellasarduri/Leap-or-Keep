...
- **B1 ⏳ 整備ドック(②-2a「金の3本足」の要)** — ⚠ 現行 credits はメタ専用。**ラン内クレジットプール新設**が要る。**場所**: §次タスク②の構造メモ。**完了**: ₢3=HP+1/₢7=ロスト回収+精算で残額メタ持越+sim不変条件。
- **B2 ⏳ オランダ式市場(①-3)** — 売れ残りスパイクの値下がり/キャッシュバック。⚠ META永続状態(未購入回数)が要る。
- **B3 ⏳ 報酬の持参金+残念賞(③-3)** — ⚠ 現行報酬は単一遺物の封印/展開で3択でない。報酬3択化 or「単一遺物+持参金」へ翻案。
