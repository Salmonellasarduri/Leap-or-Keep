...
Phase 4.5候補(未実装): P9 HUD観賞トグル / P10 下部光源リワード演出 / 遷移の意味文法 / ダイエジェティック全画面リスキン

### Phase 5 実装記録(2026-07-14) — BlenderフルGLB部屋(一気通貫パイプライン)

**「Claude→Blender(headless bpy)→GLB→three.js」パイプラインがゲーム内で実動**(記事の弾)。

- Blender: `tools/blender/salvage_desk.py --room` — Sol R1数値(暗部3段/卓面rough.79・metal.18)で部屋化→Sol初回4.2/10「ブロックアウト」
  →Sol R2トップ5適用(装甲ワークベンチ2材質+ベベル+ボルト/デスクランプ実体/縦型ガラス容器(transmission)/コンソール+状態灯8/2灯構成/カメラ38mm)
  →**Solゲート判定6.8/10(ゲート6.5通過)「雰囲気は成立」**。露出0.18が--roomの0.95を上書きする既存バグも修正
- GLB: `--room --plate` でゲームプレイ要素抜きを `art/salvage_room.glb` へ出力(export_apply=Trueでベベル適用)
- three.js: GLTFLoader+BufferGeometryUtils+SkeletonUtils を vendor/three/ にベンドル(bare specifier 'three' を相対パスへ書換=importmap不要方針の維持)
- 起動: **実験フラグ `?room=1`**(または lkDebug().META.room3d)。bg canvasに部屋(メートル空間)を読み、カメラをBlender一人称位置(38mm/eye(0,1.62,1.58))へ切替。
  ライトはthree側で近似(spot琥珀+Hemisphere+point×3)。ACESトーンマッピング(bg canvas独立=盤ホロ無影響)。失敗は2D画へ自動フォールバック
- ハマった穴: bgカメラの near=10(px用)がメートル空間では10m=部屋全体がニアクリップで不可視 → roomCamで near0.05/far60 に切替
- **既定は2D画(8.5/10)のまま**: Sol裁定「6.8はゲート通過だが2D超えには黒潰れ解消と主役の情報密度が要る」。部屋の磨きは次段(Sol反復)
- 副産物の修正: holo3クラス付与時のCSSレイアウト変化で盤キャリブが次renderまで8.4pxずれる潜在バグ → bgShowでrAF再同期(全モード恩恵)

検証: sim294+agent305 PASS / 盤整合0.31px(通常・部屋両モード) / e2e 0.03px / fpsゲートPASS(注: 計測はマシン負荷に敏感 — fps.mjs冒頭の注意書き参照)
