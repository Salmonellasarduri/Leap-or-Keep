...
node agent/cli.mjs new --seed 1129 --ship astra [--contracts heavy,minefield] [--memory <carryovers.json>] [--undo-limit 2] --file <run.json>
...
- AI入口の`undo`はボス区間ごとに既定2回まで。観測に`AI undo残り n/2`が出る。`--undo-limit 0`で禁止、`--undo-limit unlimited`で旧来の無制限。
